    ...
    <section className="bg-[#1d1d1f] py-16 md:py-24">
      <div className="max-w-[1200px] mx-auto px-4">
        ...
        <div className="flex flex-col lg:flex-row gap-8 lg:gap-16 items-center">
          {/* Feature Tabs */}
          <div className="flex flex-row lg:flex-col gap-2 overflow-x-auto lg:overflow-visible pb-4 lg:pb-0 shrink-0">
            {features.map((feature) => (
              <button
                key={feature.id}
                onClick={() => setActiveFeature(feature.id)}
                className={`flex items-center gap-2 px-4 py-2.5 rounded-full text-sm whitespace-nowrap transition-all duration-300 border ${
                  activeFeature === feature.id
                    ? "bg-[#2a2a2c] text-[#f5f5f7] border-[#d4742c] shadow-[0_0_12px_rgba(212,116,44,0.4)]"
                    : "bg-[#2a2a2c] text-[#86868b] border-[#424245] hover:bg-[#3a3a3c] hover:text-[#f5f5f7]"
                }`}
              >
                <span
                ...
                >
                  {activeFeature === feature.id ? (
                    ...
                    <Plus className="w-2.5 h-2.5 text-[#86868b]" />
                  )}
                </span>
                {feature.label}
              </button>
